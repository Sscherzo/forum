...
public class CommentController {
	...
	public String commentReply(CommentDTO commentDTO,HttpSession session) throws Exception {
		//-- 로그인을 하지 않으면 답변하지 못함
		if((String)session.getAttribute("id") == null) return "fail";		
	...
	public String commentModify(CommentDTO commentDTO,HttpSession session) throws Exception {		
		...
		//-- 로그인을 하지 않으면 수정 불가
		if((String)session.getAttribute("id") == null) return "login";
		...
		if(connectId.equals(CommentdId) == false) return "fail";	
		if(commentDTO.getComment_content() == null || commentDTO.getComment_content() == "") return "comment_content";
	...
	public String commentDelete(Model model,CommentDTO commentDTO,HttpSession session) throws Exception {	
	...
	public String commentWrite(Model model,CommentDTO commentDTO,HttpServletRequest request) throws Exception {
		
		int seq =  Integer.parseInt(request.getParameter("seq"));
		String id = request.getParameter("id");
		
		//-- 아이디가 있어야 작성 가능 , 내용을 작성해야함
		if(id == null || id == "") return "fail";		
		if(commentDTO.getComment_content() == null || commentDTO.getComment_content() == "" ) return "comment";			
