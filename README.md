...
```
작성일은 to_char 함수를 사용함으로써 원하는 형식에 맞게 바꾸었습니다.
...
페이징을 처리하는 View에서는 시작페이지가 보여줄 하단 페이지 링크수 보다 크면 [**이전**]을 출력하고 [**이전**] 링크를 누르면 (시작페이지 - 1) 값을 페이지에 돌려줍니다.  
  
끝 페이지가 총 페이지보다 작을 경우 [**다음**]을 출력하고  [**다음**]
